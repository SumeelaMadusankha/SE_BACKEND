...
async function addWorkoutPlan(req, res) {
    ...
    const result = await WorkoutPlanModel.workoutplanModel.create({note:note,targetPart:targetPart,Cur_Weight:Cur_Weight,Tar_Weight:Tar_Weight,Tar_days:Tar_days,PractiseDaysPerWeek:PractiseDaysPerWeek});
    return result;
...
async function fetchWorkoutPlan (userId){
    ...
    const workoutPlan = await WorkoutPlanModel.workoutplanModel.findByPk(userId);
